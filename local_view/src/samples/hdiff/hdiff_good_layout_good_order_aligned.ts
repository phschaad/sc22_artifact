...
export class HdiffGoodLayoutGoodOrderAligned extends Program {

    constructor(stage: Container, x: number, y: number) {
        super(stage, x, y);

        this.addGraph(HdiffGoodLayoutGoodOrderAligned.generateGraph());
    }
    ...
    public static generateGraph(): Graph {
        ...
        const infield = new DataContainer(
            'in_field [K, I+4, J+4]', [K, Ip4, Jp4], false, 8, 0, 0,
            [
                new DataDimension(
                    '(I+4+3)*(J+4+3)',
                    (Jp4.value + 3) * (Ip4.value + 3)
                ),
                new DataDimension('J+4+3', Jp4.value + 3),
                new DataDimension('1', 1),
            ]
        );
        const outfield = new DataContainer(
            'out_field [K, I, J]', [K, I, J], false, 8
        );
        const coeff = new DataContainer(
            'coeff [K, I, J]', [K, I, J], false, 8
        );
