...
export class HdiffBadLayoutBadOrder extends Program {

    constructor(stage: Container, x: number, y: number) {
        super(stage, x, y);

        this.addGraph(HdiffBadLayoutBadOrder.generateGraph());
    }
    ...
    public static generateGraph(): Graph {
        ...
        const infield = new DataContainer('in_field [I+4, J+4, K]', [Ip4, Jp4, K], false, 8);
        const outfield = new DataContainer('out_field [I, J, K]', [I, J, K], false, 8);
        const coeff = new DataContainer('coeff [I, J, K]', [I, J, K], false, 8);
        ...
        map.position.set(450, 500);

        outfieldMem.position.set((map.position.x + (map.width / 2)) - (outfieldMem.unscaledWidth / 2), 860);
